    ...
    if submit:
        ...
        if not location_data == (None, None):
            ...
            if len(route_accidents) > 0:
                st.write(f"⚠️ Atención: Se han detectado {len(route_accidents)} accidentes históricos DIRECTAMENTE sobre tu ruta.")
            else:
                st.write("La ruta parece limpia de accidentes históricos registrados (con el margen seleccionado).")
